...
class Dendrogram():
    ...
    def add_column_metadata_from_file(self, column_metadata_file, delimiter=",", header=True):
        ...
        if database:        
            column_metadata,header = self.database.get_column_metadata()
        else:
...
class Cluster():
    ...
    def get_matrix_data(self,dtb, genes,samples,table):
        dtb.verbose_mode()
        rows = dtb.get_matrix_data(genes,samples,table,datatype="expression")
        self.read_data(rows,header=False,missing_value=False,datatype="numeric")
...
def _process_(arguments):
    ...
    if arguments.dbtrue:
        ...
        c.get_matrix_data(dtb, arguments.genes,arguments.samples,arguments.table)
    else:
