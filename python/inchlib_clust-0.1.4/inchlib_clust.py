...
'''
This version of inclhlib_clust is reworked by UPSC Umeå university to allow 
the use of the internal database system for fetching data and metadata.


Author: Dr. David Sundell
E-mail: davve_2@yahoo.se (bioinformatics@upsc.se)
Umeå university


The script allows a list of genes to be directly added or the use of a 
fingerprint to fetch genes from the UPSC internal genelist.

The database mod can be activated by using -db or --dbtrue.

When the database mod is activated several required inputs are needed.

--database_pwfile (-dbp)    - A file containing the username and password for the MYSQL database. 
--database                  - The database to connect to.
--table                     - The table that will be used to fetch data
--samples                   - The sample from which to include data

--fingerprint               - The fingerprint will fetch genes from the current active genelist.
OR
--genes                     - Manually add genes as a list of gene ids separated by comma.

optional

--host                      - Set mysql host, if the mysql internal host is not default (127.0.0.1).
...
'''Handles import error in case the database module is not installed'''
try:
...
class Dendrogram():
    ...
    def __init__(self, clustering,database=False):
    ...
    def create_cluster_heatmap(self, compress=False, compressed_value="median", write_data=True,samples=False):
        ...
        #if samples:
        #    self.header = samples.split(",")
        if self.header and write_data:
            self.dendrogram["data"]["feature_names"] = [h.strip() for h in self.header]
    ...
    def __read_metadata_file__(self, metadata_file, delimiter, header, database=False):
        if self.database:
            metadata,metadata_header = database.read_metadata_file()
        else:
    ...
    def add_column_metadata_from_file(self, column_metadata_file=False, delimiter=",", header=True):
        """Adds column metadata from csv file or a database.. Column metadata doesn't have header."""
        if self.database:
            column_metadata,header = arguments.samples,arguments.samples #self.database.get_column_metadata()
        else:
    ...
    def add_alternative_data_from_file(self, alternative_data_file, delimiter, header, alternative_data_compressed_value):
        """Adds alternative_data from csv file or database."""
        if self.database:        
            alternative_data = self.database.get_alternative_column_metadata()
        else:
            self.alternative_data_compressed_value = alternative_data_compressed_value    
            alternativ_data = self.__read_alternative_data_file__(alternative_data_file, delimiter)
            print(alternative_data)
        self.add_alternative_data(alternative_data, header, alternative_data_compressed_value)
...
class Cluster():
    ...
    def __init__(self,fingerprint=False):
        self.fingerprint = fingerprint
        self.write_original = False
        
        
    def get_genes(self,fingerprint):
        ''''''
        query = '''
            SELECT genebaskets.genelist 
            FROM defaultgenebaskets 
                LEFT JOIN genebaskets 
                ON (defaultgenebaskets.gene_basket_id = genebaskets.gene_basket_id)
            WHERE defaultgenebaskets.ip = "%s"
        ''' % fingerprint
        #if self.verbose_mode:
        #    print(query)
        res = self.database.query(query)
        self.genes = res[0][0].split(",")
        return self.genes
    ...
    def read_data_from_database(self):
        rows = False## Matrix from data
        pass

    def read_csv(self, filename, delimiter=",", header=True, missing_value=False, datatype="numeric"):
        ...
        #print(rows)
        #exit()
        #print(header,missing_value,datatype)
        self.read_data(rows, header, missing_value, datatype)

    def get_matrix_data(self,dtb, genes,samples,table,sc):
        #dtb.verbose_mode()
        #datatype="cast(log2 as decimal(5,4))
        rows = dtb.get_matrix_data(genes,samples,table,datatype=sc)
        #print(rows)
        #print("-----")
        self.read_data(rows,header=True,missing_value=False,datatype="numeric")


    def read_data(self, rows, header=True, missing_value=False, datatype="numeric"):
        ...
        if self.header:
            ...
            self.header = rows[0][0:]
            #print(self.header)
            data_start = 1
        
        self.data_names = [str(row[0]) for row in rows[data_start:]]
        ''' Comment David: This does not allow a dataset without names as first value in columns'''
        self.data = [row[1:] for row in rows[data_start:]] 
        ## Pure data matrix as fetched from database, save as original_data and duplicate to self.data (deepcopy)
        ...
        if self.missing_value: ##??
            ...
            ### Why make another copy, there is already a deep copy called original data that an be used?
            #self.original_data = self.__return_missing_values__(copy.deepcopy(self.data), self.missing_values_indexes)
            self.original_data = self.__return_missing_values__(self.original_data, self.missing_values_indexes)

        ### missing values should be gone and map can be used?
...
def _process_(arguments):
    c = Cluster()
    if arguments.dbtrue:
        ### Use fingerprint for fetching genes to avoid loss of genes in case of a large list of genes.
        ### The fingerprint option will fetch genes directly from the database.
        databases = {
                "fingerprint" : "eucgenie_egrandis_v2_beta"
        }

        dtb = db.Database(arguments.database_pwfile,arguments.database)
        c.set_database(dtb)
        #dtb.verbose_mode()
        #print(glistdb)
        if not arguments.genes:
            glistdb = db.Database(arguments.database_pwfile,databases[arguments.domain])
            c.set_database(glistdb)
            arguments.genes = c.get_genes(arguments.fingerprint)
            #print(arguments.genes)
            c.set_database(dtb)
            del glistdb
        '''function replaces the function of read_csv for database usage'''
        #arguments.genes=arguments.genes.split(",")
        #print(arguments.genes)
        #lines = []
        #with open(arguments.genes) as file:
        #    lines = [line.strip() for line in file]
        #print(lines) 
        list_of_lists = []
        with open(arguments.genes) as f:
            for line in f:
                inner_list = [elt.strip() for elt in line.split(',')]
            list_of_lists=inner_list
        c.get_matrix_data(dtb, list_of_lists,arguments.samples,arguments.table,arguments.scaling)
    else:
    ...
    d = Dendrogram(c,arguments.dbtrue)
    d.create_cluster_heatmap(compress=arguments.compress, compressed_value=arguments.compressed_value, write_data=not arguments.dont_write_data,samples=arguments.samples)
    ...
    if arguments.column_metadata:
        if arguments.dbtrue:
            d.add_column_metadata_from_file()  # Instead of adding arguments, use the sample input from the database.
        else:
...
if __name__ == '__main__':
    ...
    databaseArgs = parser.add_argument_group('Database settings')
    databaseArgs.add_argument("-db",    "--dbtrue",           default=False,          help="required to allow database input", action="store_true")
    databaseArgs.add_argument("-dbp",   "--database_pwfile",    default=False,          metavar = "", type=str, help="A full path to the password file containing username and password for mysql database access")
    databaseArgs.add_argument("-dbs",   "--database",           default=False,          metavar = "", type=str, help="Required if database is not supplied in the settings file or set a different database")
    databaseArgs.add_argument("-dbtb",  "--table",              default=False,          metavar = "", type=str, help="Required if database is not supplied in the settings file or set a different database")
    databaseArgs.add_argument("--host",               default="127.0.0.1",    metavar = "", type=str, help="set database host default 127.0.0.1 ")
    databaseArgs.add_argument("-g",     "--genes",              default=False,          metavar= "",  type=str, help="Add genes to be put in the heatmap")
    databaseArgs.add_argument("-s",     "--samples",            default=False,          metavar= "",  type=str, help="Add samples to be put in the heatmap")
    databaseArgs.add_argument("-ec",     "--expression_column",            default="expression",          metavar= "",  type=str, help="if expression column is not named expression in the table")
    databaseArgs.add_argument("-f",     "--fingerprint",              default=False,          metavar= "",  type=str, help="Add fingerprint to fetch genes from active genelist")
    databaseArgs.add_argument("-d",     "--domain",              default=False,          metavar= "",  type=str, help="domain from which to fetch genes")
    databaseArgs.add_argument("-sc", "--scaling", type=str, default="vst", help="vst or scale")
